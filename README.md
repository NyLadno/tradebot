...
---
# Свеженькое
**Статус на 22.08.2026**: honest walk-forward (2024-01 → 2026-06, 26 окон) показал,
что текущие боевые `entry_zscore`/`spread_window`/`max_hold_days` (2.5/2500/15) не
подтверждены out-of-sample — итог близок к безубыточности (PF≈1.0), а не к устойчивой
прибыли. Единственная правка с достаточным основанием внесена в аварийный дефолт
(`FALLBACK_PARAMS["stop_zscore"]` 4.5→5.0). Тест Йохансена и разбивка коинтеграции
по подпериодам ещё не реализованы в `cointegration.py` — следующий шаг перед тем,
как доверять параметрам сильнее. Подробности и числа — в `PROJECT_STATUS.md` (не в
этом репозитории).
